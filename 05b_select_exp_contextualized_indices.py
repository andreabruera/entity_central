...
for s in range(1, 34):
    ...
    for stim, ordered_idxs in rankings.items():
        ...
        coeffs = [math.e**(1-c) for c in zero_one_norm(ordered_idxs)]
        #try:
        ...
        vec = numpy.average([vec*coeff for vec, coeff in zip(vecs, coeffs)], axis=0)
...
out_file = os.path.join(
                        out_folder, 
                        'exp_{}_{}_gold_{}_{}_average_{}_{}_exp_vectors.tsv'.format(
                                   args.experiment_id, 
                                   computational_model, 
                                   args.time_window, 
                                   args.language, 
